...
for enemy in range(1, 9):
    ...
    env.update_parameter(name="enemies", value=[enemy])
    ...
    for _ in range(3):
        ...
        data = [run_time, enemy, 0, np.round(np.max(population_f), 3), np.round(np.mean(population_f), 3), np.round(np.std(population_f), 3)]
        ...
        for generation in range(1, hyperp["generations"] + 1):
            ...
            data = [run_time, enemy, generation, np.round(np.max(population_f), 3), np.round(np.mean(population_f), 3), np.round(np.std(population_f), 3)]
            data_handler.add_log(data)
            print("\t{:<20} {:<8} {:<12} {:<10} {:<15} {:<12}".format(*data))
        ...
        if data_handler.champions[f"enemy {enemy}"]["fitness"] < battle_results["fitness"]:

            data_handler.champions[f"enemy {enemy}"]["run"] = run_time
            data_handler.champions[f"enemy {enemy}"]["fitness"] = battle_results["fitness"].tolist()
            data_handler.champions[f"enemy {enemy}"]["weights"] = battle_results["weights"].tolist()
            data_handler.champions[f"enemy {enemy}"]["battle time"] = t
            data_handler.champions[f"enemy {enemy}"]["victorious"] = p > e
            data_handler.champions[f"enemy {enemy}"]["hyper parameters"] = hyperp

            print(2 * "\n" + 7 * "-" + " New Champion " + 7 * "-", end="\n\n")
            print(f"\tFitness:\t{data_handler.champions[f"enemy {enemy}"]['fitness']}")
            print(f"\tVictorious:\t{data_handler.champions[f"enemy {enemy}"]['victorious']}")
            print(f"\tbattle time:\t{data_handler.champions[f"enemy {enemy}"]['battle time']}")
